...
def fill_donor_address(donor, ipnObj):
    ...
    if not donor.addresscountry:
        try:
            ...
            viewutil.tracker_log('paypal', 'Country code {0} not found'.format(ipnObj.address_country_code), get_ipn_donation(ipnObj).event) 
...
def initialize_paypal_donation(ipnObj):
    ...
    if donation.event.paypalemail != ipnObj.receiver_email:
        ipnObj.set_flag("receiver_email on ipn ({0}) does not match event ({1})".format(ipnObj.receiver_email, donation.event.paypalemail))
        ipnObj.save()
